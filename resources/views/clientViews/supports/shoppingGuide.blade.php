@extends('layouts.master')
@section('content')
    <link href="css/css-policy.css" rel="stylesheet">
    <div class="inner-header">
        <div class="container">
            <div class="pull-left">
                <h2 class="inner-title main-color">Hướng Dẫn Mua Hàng</h2>
            </div>
            <div class="pull-right">
                <div class="beta-breadcrumb font-large">
                    <a href="index">Trang Chủ</a> / <span>Hướng dẫn mua hàng</span>
                </div>
            </div>
            <div class="clearfix"></div>
        </div>
        <br>
    </div>
    <div class="first-div">
        <p class="text-size">Quý khách xem thông tin sản phẩm trên website, lựa chọn sản phẩm yêu thích,<b>nhấn vào nút
                “Thêm Giỏ Hàng”, sản phẩm sẽ được cho vào giỏ hàng.</b> Quý khách có thể lựa chọn tiếp sản phẩm cần mua.
        </p>
        <img class="image-addtocart" src="source/image/supports/addToCart.png">
        <p class="text-size">Sau khi đã thêm cac sản phẩm bạn muốn mua vào giỏ hàng hãy ân nút "Đặt Hàng" trên giỏ
            hàng.Để tiến hành mua hàng</p>
        <div class="image-form"><img class="image-addtocart" src="source/image/supports/buy.png"></div>
        <p class="text-size">Sau khi điền đầy đủ và chính xác thông tin hãy chọn phương thức thanh toán mà bạn muốn</p>
        <h4>I. GIAO HÀNG</h4>
        <p class="text-size"> Theo biểu phí của công ty chuyển phát. Chúng tôi sẽ thông báo phí giao hàng chính xác cho quý khách khi xác nhận đơn hàng.</p>
        <h4>II. THANH TOÁN</h4>
        <p class="text-size">Chúng tôi chấp nhận nhận hàng bằng hình thức COD(trả tiền khi thanh toán) và chuyển khoản qua ngân hàng</b> </p>
        <br>
        <h5><b>*Ngân Hàng TP Bank</b></h5>
        <p class="text-size">Số TK:00450545502</p>
        <p class="text-size">Tên: Nguyễn Văn Dũng</p>
        <br>
        <h5><b>*Ngân Hàng Agribank</b></h5>
        <p class="text-size">Số TK:2704205133910</p>
        <p class="text-size">Tên: Nguyễn Văn Dũng</p>
        <br>
        <h3><b>Chân thành cảm ơn Quý khách đã quan tâm đến các sản phẩm nhãn hiệu The Pet Family</b></h3>
        <br>
    </div>
@endsection